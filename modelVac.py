...
class ExtSEIRSNetworkModelVac ():
    ...
    def __init__(self, G, beta, sigma, lamda, gamma,
        ...
        self.transitions =  {
                                ...
                                #---------------------------------------------------------------------
                                'StoR':         {'currentState':self.S,       'newState':self.R},
                                'EtoR':         {'currentState':self.E,       'newState':self.R},
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def calc_propensities(self):
        ...
        propensities_StoE       = (1-qu)*( self.alpha *
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            propensities_EtoIPRE     = (1-qu)*self.sigma * (self.X==self.E)

            propensities_IPREtoISYM  = (1-qu)*self.lamda * ((self.X==self.I_pre) & (numpy.greater_equal(self.rand_a, self.a)))

            propensities_IPREtoIASYM = (1-qu)*self.lamda * ((self.X==self.I_pre) & (numpy.less(self.rand_a, self.a)))
            ...
            propensities_IASYMtoR    = ((1-qu)*self.gamma_asym * (self.X==self.I_asym))+qu*fi*(self.X==self.I_asym)
            ...
            propensities_StoR        = qu*fi*(self.X==self.S)

            propensities_EtoR        = qu*fi*(self.X==self.E)

            propensities_IPREtoR     = qu*fi*(self.X==self.I_pre)
        ...
        propensities = numpy.hstack([propensities_StoE, propensities_EtoIPRE, propensities_IPREtoISYM, propensities_IPREtoIASYM,
                                     ...
                                     propensities_StoR,propensities_EtoR,propensities_IPREtoR,
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def introduce_exposures(self, num_new_exposures):
        ...
        for exposedNode in exposedNodes:
            if(self.X[exposedNode]==self.S):
                self.X[exposedNode] = self.R
            elif(self.X[exposedNode]==self.Q_S):
                self.X[exposedNode] = self.Q_E

    def introduce_vaccined(self,num_new_vaccined):
         vaccinedNodes = numpy.random.choice(range(self.numNodes), size=num_new_vaccined, replace=False)
         for vaccinedNode in vaccinedNodes:
            self.X[vaccinedNode] = self.R
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None, plot_S='line', plot_E='line', plot_I_pre='line', plot_I_sym='line', plot_I_asym='line',
        ...
        # Prepare data series to be plotted:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        Sseries      = self.numS/self.numNodes if plot_percentages else self.numS*1000
        Eseries      = self.numE/self.numNodes if plot_percentages else self.numE*1000
        I_preseries  = self.numI_pre/self.numNodes if plot_percentages else self.numI_pre*1000
        I_symseries  = self.numI_sym/self.numNodes if plot_percentages else self.numI_sym*1000
        I_asymseries = self.numI_asym/self.numNodes if plot_percentages else self.numI_asym*1000
        Rseries      = self.numR/self.numNodes if plot_percentages else self.numR*1000
        Hseries      = self.numH/self.numNodes if plot_percentages else self.numH*1000
        Fseries      = self.numF/self.numNodes if plot_percentages else self.numF*1000
